<%@ Page Title="" Language="C#" MasterPageFile="~/System/Backend/admin.Master" AutoEventWireup="true"
    CodeBehind="CityList.aspx.cs" EnableEventValidation="false" Inherits="System.Backend.CityList" %>
...
<asp:Content ID="Content3" ContentPlaceHolderID="CPHPageHeader" runat="server">
    <img src="../lib/icons/32/Dashboard.png" class="imgIcon" /><asp:Label runat="server"
        Text="مدن المخدومين" CssClass="tdMainTitle" ID="lblPageMainTitle"></asp:Label>
</asp:Content>
    ...
    <table width="100%" cellpadding="2" cellspacing="2">
        <tr>
            <td class="tdHeader">
                ...
                <asp:Label runat="server" Text="قائمة مدن المخدومين" CssClass="tdPageTitle" ID="lblPageSubTitle"></asp:Label>
                <asp:HyperLink runat="server" class="addnew" ID="btnAddNew" NavigateUrl="~/System/Backend/City/CityManagement.aspx"
                            ...
                            <ItemTemplate>
                                <div id="dvUp" style='width: 16px; height: 16px; float: left; display: <%# ShowArrow(Eval("RecOrder"), "U") %>'>
                                    <asp:ImageButton ID="ImgArrowUp" CssClass="btnArrUp" runat="server" CommandName="ArrowUp"
                                        CommandArgument='<%# Eval("CityId") %>' ImageUrl="~/App_Themes/Default/Images/Up.png"></asp:ImageButton>
                                </div>
                                <div id="dvDown" style='width: 16px; height: 16px; float: left; display: <%# ShowArrow(Eval("RecOrder"), "D") %>;'>
                                    <asp:ImageButton ID="ImgArrowDown" runat="server" CssClass="btnArrDown" CommandName="ArrowDown"
                                        CommandArgument='<%# Eval("CityId") %>' ImageUrl="~/App_Themes/Default/Images/Down.png"></asp:ImageButton>
                                </div>
                            </ItemTemplate>
                            <HeaderStyle Width="10%" />
                            <ItemStyle HorizontalAlign="Center" Width="10%" />
                        </asp:TemplateField>
                        <asp:BoundField DataField="CityName" HeaderText="اسم المدينة" SortExpression="CityName">
                            <HeaderStyle Width="75%" />
                            <ItemStyle Width="75%" />
                        </asp:BoundField>
                        <asp:TemplateField>
                            <ItemTemplate>
                                <asp:HyperLink ID="imgEdit" runat="server" CssClass="imgIcon3 tip_right_top" NavigateUrl='<%#"CityManagement.aspx?id="+Eval("CityId")%>'
                                ...
                                <asp:ImageButton ID="imgRestore" runat="server" CssClass="imgIcon3 tip_right_top"
                                    ...
                                    CommandArgument='<%# Eval("CityId") %>' Visible='<%# DAL.GeneralMethods.DeleteRestorVisible(Eval("Active"),"true") %>'
                                ...
                                <asp:ImageButton ID="imgDelete" runat="server" CssClass="imgIcon3 tip_right_top" ImageUrl="../lib/img/Delete.png"
                                    CommandName="deleteitem" CommandArgument='<%# Eval("CityId") %>' OnClientClick="if(!confirm('Are you sure you want delete this?')) return false;"
